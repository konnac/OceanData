...
static const float ParamMin[] = { -5.0f, 0.0f, 6.5f, 0.0f, 0.0f, -10.0f };
static const float ParamMax[] = { 40.0f, 45.0f, 9.0f, 15.0f, 500.0f, 50.0f };
...
int IsParamValid(ParamType type, float value) {
    if (type < 0 || type > PARAM_AIRTEMP) return 0;
    return (value >= ParamMin[type] && value <= ParamMax[type]);
}
